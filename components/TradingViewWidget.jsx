...
function TradingViewWidget({ crypto}) {
  ...
  const timeFrames = ["1H", "24H", "7D", "1M", "3M", "6M", "1Y", "ALL"];
    ...
    useEffect(() => {
        ...
        const widgetHeight = window.innerWidth < 500 ? "217" : "400";
        const symbolMap = {
  ...
  return (
    <>
      <div className="flex flex-row justify-between max-sm:hidden">
        ...
        <div>
          <button className="text-[13px] font-[500] text-[#5D667B] leading-[19.5px] flex items-center gap-[8px]">
            {timeFrames.map((timeFrame, index) => (
              <div
                key={index}
                className={`p-2 ${
                  selectedTimeFrame === timeFrame ? "bg-[#E2ECFE] rounded-[15px]  text-[#0141CF]" : ""
                }`}
